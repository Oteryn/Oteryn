...
- Protected-integration scheduling: before releasing a mutating worker expected to
  ...
  current-session capability evidence assembled from live tool discovery and
  protected-executor readback. Raw mappings/JSON, task prose and stale prior-session
  assertions are not authority. Only `DIRECT_CAPABLE` or verified
  `DELEGATED_CAPABLE` permits that worker release; capability is not merge authority.
  Missing, malformed or stale evidence fails closed.
