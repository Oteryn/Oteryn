...
class FakeObserver(routing.TrustedCapabilityObserver):
    """Compatibility acquisition helper used to prove legacy callers still work."""
...
def state(source: object) -> str:
    return routing.observe_and_classify(source, policy(), now_epoch_seconds=NOW)
...
def test_non_integrating_task_is_not_blocked_with_fresh_typed_evidence() -> None:
...
def test_direct_native_operation_accepts_fresh_typed_evidence() -> None:
...
def test_legacy_observer_adapter_remains_compatible() -> None:
    current = routing.AcquiredCapabilityEvidence(
        True, NOW, ("github.merge_async.put_exact_head",), ()
    )
    assert state(FakeObserver(current)) == routing.DIRECT_CAPABLE


def test_verified_delegated_executor_accepts_fresh_typed_evidence() -> None:
    current = routing.AcquiredCapabilityEvidence(
...
def test_delegated_request_retains_classifier_bound_canary_sha() -> None:
    ...
    decision = routing.observe_and_decide(current, policy(), now_epoch_seconds=NOW)
...
def test_forged_decision_or_sha_cannot_replace_classifier_bound_canary() -> None:
    for forged in (
...
def test_unverified_delegated_executor_is_not_capability() -> None:
    ...
    assert "before worker release" in routing.validate_worker_release(current, policy(), now_epoch_seconds=NOW)[0]


def test_serialized_dictionary_cannot_authorize_direct_worker_release() -> None:
    forged = {
...
def test_serialized_dictionary_cannot_authorize_delegated_worker_release() -> None:
    expected = policy()["integration_capability_routing"]["protected_executor"]
    forged = {
...
def test_stale_and_future_typed_evidence_fails_closed() -> None:
...
def test_delegated_route_rejects_invalid_executor_readback() -> None:
    for changes in (
        ...
        {"canary_protected_main_sha": "a" * 40}, {"credential_principal": ""},
    ):
        current = routing.AcquiredCapabilityEvidence(
            True, NOW, ("github.issue_comment.create",),
...
def test_current_session_adapter_still_acquires_live_typed_evidence() -> None:
    class Tools:
...
def test_malformed_policy_cannot_authorize_a_route() -> None:
    for mutate in (
        ...
        try:
            routing.observe_and_classify(evidence(), candidate, now_epoch_seconds=NOW)
        except ValueError:
            pass
        else:
