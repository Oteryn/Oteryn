...
def test_policy_is_closed_and_targets_all_permanent_repositories() -> None:
    ...
    assert cfg["allowed_target_repositories"] == {
...
def test_non_integrating_task_is_not_blocked() -> None:
    assert routing.classify(snapshot(required=False), policy()) == routing.NOT_REQUIRED
    assert routing.validate_worker_release(snapshot(required=False), policy()) == []
...
def test_direct_native_operation_is_preferred() -> None:
    ...
    assert routing.classify(current, policy()) == routing.DIRECT_CAPABLE
    assert routing.validate_worker_release(current, policy()) == []
...
def test_verified_delegated_executor_prevents_late_worker_block() -> None:
    ...
    current["operational_executor_routes"] = ["meta.governed_merge_queue_executor.v1"]
    assert routing.classify(current, policy()) == routing.DELEGATED_CAPABLE
    assert routing.validate_worker_release(current, policy()) == []
...
def test_unverified_delegated_executor_is_not_capability() -> None:
    current = snapshot()
    current["available_operations"] = ["github.issue_comment.create"]
    assert routing.classify(current, policy()) == routing.BLOCKED_CAPABILITY_UNAVAILABLE
    errors = routing.validate_worker_release(current, policy())
...
def test_malformed_policy_cannot_authorize_a_route() -> None:
    ...
    for candidate in cases:
        ...
        try:
            routing.classify(snapshot(), candidate)
        except ValueError:
            pass
        else:
