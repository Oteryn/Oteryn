...
def test_current_codex_completed_summary_plus_exact_clean_review_passes_without_reaction() -> None:
    repo, _, final = _v1.core_tests.make_repo()
    found = _verify_summary(
        reactions=[],
...
def test_current_codex_summary_requires_trusted_completion_evidence() -> None:
    ...
    _v1.core_tests.expect_fail(
        lambda: _verify_summary(
            reactions=[],
            extra_reviews=[_p2_review(final, login="untrusted-bot", body=_codex_review_envelope(final))],
        )
    )
