...
def _validate_history(
    ...
    for field in RETRY_COUNTER_FIELDS:
        if _counter_scope(previous, field) == _counter_scope(current, field) and current[field] < previous[field]:
    ...
    opened_repair = previous["candidate_frozen"] and not current["candidate_frozen"]
    replaced_frozen_head = previous["candidate_frozen"] and head_changed
    post_freeze_delta = current["post_freeze_material_head_changes"] - previous["post_freeze_material_head_changes"]
    if opened_repair or replaced_frozen_head:
        if not current["material_change"]:
            raise GuardError("opening a post-freeze repair generation requires material_change=true")
        if post_freeze_delta != 1:
            raise GuardError(
                "opening a post-freeze repair generation must increment post_freeze_material_head_changes exactly once"
            )
    elif post_freeze_delta != 0:
        raise GuardError(
            "post_freeze_material_head_changes may increase only when a frozen candidate opens a material repair generation"
        )
...
def _action_counter_decision(
    ...
    if action == "retry":
        if not current["first_material_failure"]:
            return None
        field = "identical_failure_cycles"
        budget = policy["retry_budgets"]["identical_failure_cycles"]
        if current[field] >= budget:
            return _decision(
                False,
                "STALLED",
                "identical failure retry budget is exhausted without material progress",
                True,
                progress,
                failure,
            )
        denied_state, release = "STALLED", True
    elif action in ACTION_COUNTERS:
        field, budget_key = ACTION_COUNTERS[action]
        budget = policy["retry_budgets"][budget_key]
        denied_state = "WAITING_EXTERNAL" if action in {"request_external_review", "same_head_gate_recheck"} else "STALLED"
        release = True
        if current[field] > budget:
            return _decision(False, denied_state, f"{field} budget is exhausted", release, progress, failure)
    else:
        return None
    ...
    if previous is None:
        return _decision(
            False,
            denied_state,
            f"authoritative previous snapshot is required before consuming the {field} counter",
            release,
            progress,
            failure,
        )

    previous_value = previous[field] if _counter_scope(previous, field) == _counter_scope(current, field) else 0
    if current[field] != previous_value + 1:
        return _decision(
            False,
            denied_state,
            f"{field} counter must be consumed exactly once relative to the authoritative previous snapshot",
            release,
            progress,
            failure,
        )

    if action != "retry" and current[field] > budget:
        return _decision(False, denied_state, f"{field} budget is exhausted", release, progress, failure)
    return None
...
def decide(
    ...
    if current["state"] == "WAITING_EXTERNAL":
        if requested_action == "observe":
            return _decision(True, "WAITING_EXTERNAL", "waiting task may observe for a material external change", True, progress, failure)
        if requested_action != "complete":
            return _decision(
                False,
                "WAITING_EXTERNAL",
                "WAITING_EXTERNAL is operationally terminal until a material fact changes",
                True,
                progress,
                failure,
            )
    ...
    if requested_action == "enter_final_qualification" and not current["candidate_frozen"]:
        return _decision(False, "READY", "final qualification requires a frozen technical candidate", False, progress, failure)
    ...
    if current["candidate_frozen"] and requested_action in forbidden_when_frozen and not current["material_change"]:
        if current["state"] in release_states:
            ...
            state = current["state"]
        return _decision(False, state, "candidate is frozen; mutation/retrigger without a material change is forbidden", state in release_states, progress, failure)

    consumption = _action_counter_decision(previous, current, requested_action, policy, progress, failure)
