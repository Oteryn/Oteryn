...
CANONICAL_LEDGER_SHA = 'ff5c6621a78c14fc17802ecf01b4ef815867ccab95acce90c490973946d6279b'
...
def validate_adopted_docs(candidate: dict, audit_root: Path) -> None:
    ...
    require(candidate['projection']['projected_ledger_sha256']==ANNOUNCEMENTS_LEDGER_SHA,'projected ledger provenance drift')
    ...
    require(report.get('revision')=='R3-NATIVE-EVIDENCE-POST-REVIEW-PLATFORM-SEMANTIC-CARRYFORWARD-113-DIRECT-294','report revision drift')
    ...
    require(report.get('scoped_review_paths')==294 and report.get('grouped_revalidated_paths')==113 and report.get('semantically_classified_paths')==407,'report accounting drift')
    ...
    require(summary.get('scoped_review_paths')==294 and summary.get('grouped_revalidated_paths')==113 and summary.get('unverified_semantics_total')==3918 and summary.get('semantically_classified_paths')==407,'summary accounting drift')
    ...
    require(len(review)==294,'canonical DIRECT count drift')
