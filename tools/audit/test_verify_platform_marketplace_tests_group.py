...
class PlatformMarketplaceTestsAdoptedTests(unittest.TestCase):
    ...
    def test_summary_accounting_is_current_and_exact(self):
        ...
        self.assertEqual((s['scoped_review_paths'], s['grouped_revalidated_paths'], s['semantically_classified_paths'], s['unverified_semantics_total']), (223, 113, 336, 3989))
        self.assertEqual((p['direct_scoped'], p['grouped'], p['unverified_semantics']), (158, 113, 1894))

    def test_report_accounting_and_candidate_binding_are_current_and_exact(self):
        r = self.report()
        self.assertEqual(r['revision'], 'R3-NATIVE-EVIDENCE-POST-REVIEW-PLATFORM-SEMANTIC-CARRYFORWARD-113-DIRECT-223')
        self.assertEqual((r['scoped_review_paths'], r['grouped_revalidated_paths'], r['semantically_classified_paths']), (223, 113, 336))
