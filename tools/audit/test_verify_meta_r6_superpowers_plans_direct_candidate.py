#!/usr/bin/env python3
"""Adversarial tests for the META R6 historical-plans candidate."""
from copy import deepcopy
from pathlib import Path
import csv, json, shutil, tempfile, unittest
import verify_meta_r6_superpowers_plans_direct_candidate as v

class MetaR6PlansCandidateTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_semantic_rows_authority_classification_and_types_fail_closed(self):
        mutations = [lambda c: c['paths'][0].update(semantic_scope='generic historical plan'), lambda c: c['paths'][0].update(authority_classification='CURRENT_DISPATCH_AUTHORITY'), lambda c: c['paths'][5].update(limitations='Current live enforcement is proven.'), lambda c: c['paths'][0].update(full_file_sha256='0'*64), lambda c: c.update(candidate_path_count='9'), lambda c: c.update(extra_claim=True)]
        for mutation in mutations:
            with self.subTest(mutation=mutation): self.reject_candidate(mutation)
