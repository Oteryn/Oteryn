...
class AuditRecorderDirectVerifierTests(unittest.TestCase):
    ...
    def test_premature_adoption_fails_closed(self):
        c = copy.deepcopy(self.candidate); c["coverage_adopted"] = True
        with self.assertRaisesRegex(ValueError, "pre-adoption"): v.validate_candidate_shape(c)

    def test_projected_count_drift_fails_closed(self):
        c = copy.deepcopy(self.candidate); c["projected_accounting"]["direct_paths"] = 224
    ...
    def test_primary_qualification_drift_fails_closed(self):
        c = copy.deepcopy(self.candidate); c["qualification"]["workflow_run"] += 1
        with self.assertRaisesRegex(ValueError, "bound primary qualification"): v.validate_candidate_shape(c)

    def test_initial_projection_provenance_drift_fails_closed(self):
        c = copy.deepcopy(self.candidate); c["initial_projection"]["ledger_sha256"] = "0" * 64
        with self.assertRaisesRegex(ValueError, "initial projection provenance"): v.validate_candidate_shape(c)

    def test_planned_execution_evidence_drift_fails_closed(self):
        c = copy.deepcopy(self.candidate); c["planned_execution_evidence"] += " product ready"
        with self.assertRaisesRegex(ValueError, "planned execution evidence"): v.validate_candidate_shape(c)
    ...
    def test_direct_overlap_fails_closed(self):
        ...
        summary = {"scoped_review_paths":221,"grouped_revalidated_paths":113,"semantically_classified_paths":334,"unverified_semantics_total":3991,"ledger_sha256":self.candidate["baseline_accounting"]["ledger_sha256"],"per_repository":{"platform":{"leaves":2165,"direct_scoped":156,"grouped":113,"unverified_semantics":1896}}}
        report = {"scoped_review_paths":221,"grouped_revalidated_paths":113,"semantically_classified_paths":334}
        with self.assertRaisesRegex(ValueError, "already DIRECT"): v.validate_pre_adoption_docs(self.candidate, review, summary, report)
