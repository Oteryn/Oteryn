...
class AuditRecorderDirectVerifierTests(unittest.TestCase):
    ...
    def test_unearned_post_adoption_success_fails_closed(self):
        c=copy.deepcopy(self.candidate);c['post_adoption_revalidation']['status']='SUCCESS'
        with self.assertRaisesRegex(ValueError,'post-adoption state'):v.validate_candidate_shape(c)
    ...
    def test_overlay_blob_hash_is_exact(self):
        raw=(ROOT/v.OVERLAY_REL).read_bytes();self.assertEqual(v.blob_sha(raw),v.OVERLAY_BLOB)
