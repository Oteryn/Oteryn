...
This binds exact frozen source/persistence/test identities, the two-row DIRECT overlay,
canonical accounting and pre-adoption qualification provenance. A successful result
is bounded audit evidence only and never product or production-readiness PASS.
"""
...
def tree_blob(cwd: Path,path: str) -> str:
    ...
    head,actual_path=raw.split('\t',1);mode,kind,oid=head.split(' ')
...
def validate_candidate_shape(c: dict) -> None:
    ...
    require(c.get('disposition')=='DIRECT_ADOPTED_PENDING_POST_ADOPTION_PROOF','candidate disposition')
    ...
    require(c.get('baseline_accounting')=={
        'source_rows':4325,'direct_paths':221,'grouped_paths':113,'unverified_paths':3991,'semantically_classified_paths':334,
        'platform_direct_paths':156,'platform_grouped_paths':113,'platform_unverified_paths':1896,
        'ledger_sha256':'25ed5eb371279fbdb16a50263637856a3bc409b775387555efdaa17cebdc3617'},'baseline accounting drift')
    require(c.get('projected_accounting')=={
        'source_rows':4325,'direct_paths':223,'grouped_paths':113,'unverified_paths':3989,'semantically_classified_paths':336,
        'platform_direct_paths':158,'platform_grouped_paths':113,'platform_unverified_paths':1894,'ledger_sha256':LEDGER_SHA},'projected accounting drift')
    ...
    require(pre=={
        'status':'SUCCESS','head_sha':'14b18e00936cbf09884e51bfc4c8eaf121edd715','workflow_run':34348443102,'job':102455526087,
        'artifact':10102677683,'artifact_sha256':'0103eca993e0a52d2d9bab834594cb5d63072842bb40f819b34de7d47be7d7a1',
        'meta_ci_run':34348443175,'verifier_unit_tests':12,'test_files':3,'cases':25,'assertions':89,'failures':0,'errors':0,'skipped':0,
        'projected_ledger_sha256':LEDGER_SHA},'pre-adoption exact-head evidence drift')
    adoption=c.get('adoption') or {}
    require(adoption=={
        'status':'ADOPTED_PENDING_POST_ADOPTION_REVALIDATION_AND_INDEPENDENT_REVIEW',
        'direct_overlay_path':str(OVERLAY_REL),'direct_overlay_blob_sha':OVERLAY_BLOB,'adopted_paths':2,
        'source_rows':4325,'direct_paths':223,'grouped_paths':113,'unverified_paths':3989,'semantically_classified_paths':336,'ledger_sha256':LEDGER_SHA},'adoption state drift')
    post=c.get('post_adoption_revalidation') or {}
    require(post=={'status':'PENDING_EXACT_HEAD_PROOF','head_sha':None,'workflow_run':None,'job':None,'artifact':None,'artifact_sha256':None,
                   'meta_ci_run':None,'verifier_unit_tests':None,'cases':None,'assertions':None,'ledger_sha256':None},'post-adoption state must remain pending on adoption commit')
    joined=' '.join(c.get('limitations') or [])
    for phrase in ['not a product or production-readiness PASS','not an exhaustive execution','Exact-head post-adoption qualifier/ledger proof plus fresh independent review remain required']:
...
def validate_adopted_docs(c: dict,audit_root: Path) -> None:
    ...
    rows=list(csv.DictReader(overlay.decode('utf-8').splitlines(),delimiter='\t'))
    require(len(rows)==2 and {(r['repository'],r['path']) for r in rows}=={('platform',p) for p in EXPECTED_PATHS},'DIRECT overlay membership drift')
    summary=vr.read_json(audit_root/SUMMARY_REL);platform=summary['per_repository']['platform']
...
def run(audit_root: Path,platform_root: Path) -> dict:
    ...
    validate_candidate_shape(c);validate_adopted_docs(c,audit_root);validate_platform(c,platform_root)
    return {
        'result':'PLATFORM_AUDIT_RECORDERS_DIRECT_ADOPTED_PENDING_POST_PROOF_NOT_PRODUCT_PASS',
        ...
        'coverage_adopted':True,'direct_paths':223,'grouped_paths':113,'unverified_paths':3989,'ledger_sha256':LEDGER_SHA,
    }


def main() -> None:
    parser=argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--audit-root',type=Path,required=True);parser.add_argument('--platform-root',type=Path,required=True)
    args=parser.parse_args();print(json.dumps(run(args.audit_root.resolve(),args.platform_root.resolve()),indent=2,sort_keys=True))
