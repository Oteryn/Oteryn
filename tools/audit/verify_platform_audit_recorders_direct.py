...
CURRENT_LEDGER_SHA = '71f41a16273b3eb16afc0a786f2ff4d2e83b737fac0e869d9e2cf97d43483d11'
...
def validate_adopted_docs(candidate: dict, audit_root: Path) -> None:
    ...
    require(report.get('revision')=='R3-NATIVE-EVIDENCE-POST-REVIEW-PLATFORM-SEMANTIC-CARRYFORWARD-113-DIRECT-258','current report revision drift')
    ...
    require(report.get('scoped_review_paths')==258 and report.get('grouped_revalidated_paths')==113 and report.get('semantically_classified_paths')==371,'current report accounting drift')
    ...
    require(summary.get('scoped_review_paths')==258 and summary.get('grouped_revalidated_paths')==113 and summary.get('unverified_semantics_total')==3954 and summary.get('semantically_classified_paths')==371,'current summary accounting drift')
    ...
    require(len(review)==258,'current composed DIRECT count drift')
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        'recorder_paths':2,
        'current_direct_paths':258,
        'current_grouped_paths':113,
        'current_unverified_paths':3954,
        'current_semantically_classified_paths':371,
