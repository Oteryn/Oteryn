...
class AuditValidationTest(unittest.TestCase):
    ...
    def test_current_coverage_table_meta_drift_rejected(self):
        self.mutate_companion('| meta | 174 | 81 | 0 | 93 |','| meta | 174 | 45 | 0 | 129 |')
        self.reject()
    def test_current_coverage_table_total_drift_rejected(self):
        self.mutate_companion('| **Total** | **4325** | **294** | **113** | **3918** |',
                              '| **Total** | **4325** | **258** | **113** | **3954** |')
        self.reject()
    def test_current_history_annotation_present_count_drift_rejected(self):
        self.mutate_companion('present canonical 294-path state','present canonical 258-path state')
        self.reject()
    ...
    def test_stale_semantic_coverage_transition_rejected(self):
        self.mutate_semantic_coverage(lambda row:row.update(
            reason=row['reason'].replace('294 DIRECT', '258 DIRECT')
                                .replace('3918 paths', '3954 paths')))
        self.reject()
    ...
    def test_missing_recorder_provenance_rejected(self):
        self.mutate_semantic_coverage(lambda row:row.update(
            reason=row['reason'].replace('the prior 283 plus exactly 11 META prompt-family source paths', 'exactly 11 META prompt-family source paths')))
        self.reject()
    def test_missing_announcements_provenance_rejected(self):
        self.mutate_semantic_coverage(lambda row:row.update(
            reason=row['reason'].replace('the prior 283 plus exactly 11 META prompt-family source paths', 'the prior 273 plus exactly 11 META prompt-family source paths')))
        self.reject()
