...
class ReadmeCurrentStateTest(unittest.TestCase):
    ...
    def test_current_readme_passes(self):
        ...
        self.assertEqual(result['direct_paths'], 258)
        self.assertEqual(result['grouped_paths'], 113)
        self.assertEqual(result['unverified_paths'], 3954)
        self.assertEqual(result['semantically_classified_paths'], 371)
    ...
    def test_old_accounting_transition_rejected(self):
        mutated = self.current.replace('258 DIRECT scoped path reviews', '233 DIRECT scoped path reviews', 1)
        mutated = mutated.replace('3954 retain UNVERIFIED semantics', '3979 retain UNVERIFIED semantics', 1)
        self.reject(mutated)
    ...
    def test_truncated_current_accounting_paragraph_rejected(self):
        mutated = self.current.replace(' and 3954 retain UNVERIFIED semantics.', '.', 1)
        self.reject(mutated)
