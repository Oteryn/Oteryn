...
def validate_accounting(root,c,inventory_dir=None):
 ...
 expected={'source_leaves':4325,'scoped_review_paths':258,'grouped_revalidated_paths':113,'unverified_semantics':3954,'semantically_classified_paths':371}
 ...
 require(all(r['disposition']=='UNVERIFIED' for r in selected),'candidate path is already DIRECT or GROUPED')
 require(not any(r['disposition'] in {'DIRECT','GROUPED'} for r in selected),'candidate overlap with classified coverage')
 require(sum(grouped.values())==113,'GROUPED accounting drift')
 return result

def main():
 p=argparse.ArgumentParser(); p.add_argument('--audit-root',type=Path,default=ROOT); a=p.parse_args(); c=expected_candidate(a.audit_root); validate_candidate(c,c); validate_source(a.audit_root,c); validate_finding(a.audit_root); validate_accounting(a.audit_root,c)
 print(json.dumps({'result':'META_R5_INSTRUCTION_EFFICIENCY_DIRECT_CANDIDATE_VALID_NOT_ADOPTED','candidate_paths':25,'coverage_delta':0,'coverage_adopted':False,'adoption_performed':False,'current_disposition':'UNVERIFIED','eligible_adoption_depth':'SCOPED_SEMANTIC_REVIEW','source_rows':4325,'direct_paths':258,'grouped_paths':113,'unverified_paths':3954,'semantically_classified_paths':371,'ledger_sha256':LEDGER_SHA,'meta_aud_05_status':'PARTIALLY_REPAIRED','residual_obligations':14,'product_readiness_claimed':False,'audit_completion_claimed':False,'runtime_attested':False,'live_state_claimed':False},sort_keys=True)); return 0
