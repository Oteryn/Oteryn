...
def validate_accounting(audit_root: Path) -> None:
    ...
    require((summary.get('grouped_revalidated_paths'), summary.get('semantically_classified_paths'), summary.get('unverified_semantics_total')) == (113, 407, 3918), 'Marketplace-test summary totals drift')
    report = read_json(audit_root / REPORT)
    require(report.get('revision') == 'R3-NATIVE-EVIDENCE-POST-REVIEW-PLATFORM-SEMANTIC-CARRYFORWARD-113-DIRECT-294', 'Marketplace-test report revision drift')
    require((report.get('scoped_review_paths'), report.get('grouped_revalidated_paths'), report.get('semantically_classified_paths')) == (294, 113, 407), 'Marketplace-test report accounting drift')
