...
For `CONTROL_PLANE_R2`, the owner authorization must be a new, unedited top-level comment on the canonical current PR/Issue by a human whose present repository-owner authority is verified through direct GitHub readback. The direct record must bind the comment ID, author login/type and independently read owner role, `created_at == updated_at`, repository, PR, exact material head SHA, scope, and positive authorization. Bot/coordinator or ordinary-contributor text, a stale/head-mismatched, edited, missing, or unreadable comment, or an actor whose owner role cannot be verified is `UNKNOWN`, never integration authority. The V2 trusted boundary is that authenticated human owner and one serial settings writer; organization audit-log proof of exact comment creation/order is intentionally not required and must not be replaced with an attestation, audit, or proof subsystem. This is an evidence constraint on the existing owner decision, not a second-human approval, a required status, or a new identity service.

Use the existing execution-routing policy as a repository-scoped preflight, not as one organization-wide token. Execute its validator and policy only from an `Oteryn/Oteryn` checkout or trusted workflow verified at protected `main`; for Game, Platform, and Atlas, pass their own packet and fresh GitHub-state snapshot to that META authority rather than expecting or copying the mechanism into a provider checkout. Do not use a provider or candidate-controlled copy as governance authority. Immediately before each repository's implementation or live-settings phase, refresh that repository's live GitHub snapshot and validate a packet whose `execution_routing.github_preflight` contains only `verified_at`, `repository`, `default_branch_sha`, `governing_issue`, `pull_request`, and `task_head_sha`. Put execution target/runner, equivalent-CI availability, and Remote Desktop restrictions in `execution_routing`; put high-effort decision basis, lane strategy, and the single settings-writer lane in `parallel_execution`. A passing META packet cannot be reused for Game, Platform, or Atlas; a resumed or materially changed task requires a fresh validation for the affected repository. This is a bounded application of the existing execution-routing policy, adds no external required status, and is intended to prevent out-of-scope or stale-environment mutations rather than create a new autonomous merge authority.
...
During the ordered rollout, a repository whose own cutover has not begun is `PENDING` only when its canonical rollout Issue/PR contains an authoritative pending-baseline entry captured from direct live GitHub readback: `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback`. The read-only auditor recomputes and compares the fingerprint, then compares current live state to that baseline; any mismatch is `DRIFT`, not `PENDING`. The baseline and every transition record are existing top-level PR/Issue lifecycle entries whose comment IDs and direct GitHub `created_at == updated_at` state must be read back; missing, edited, duplicate, malformed, or mismatching evidence is `DRIFT`, and an unreadable required live field is `UNKNOWN`. A transition uses a distinct unedited pre-transition entry and terminal entry linked by `transition_id` and the pre-entry ID; their GitHub creation times, not values supplied in comment bodies, are the canonical start/close times. The one serial settings writer creates and reads back the pre-transition entry before mutation; this procedural order is trusted, not independently audit-proven, and a terminal record cannot validate a missing pre-transition entry. This is evidence in the existing rollout record, not a `TRANSITION` exception, settings authorization, or new database. `PENDING` carries no V2 settings deviation and cannot be counted as `TARGET`; it becomes `TRANSITION` only after a fresh matching readback and that repository's bounded receipt are created immediately before its serial cutover. A failed cutover becomes terminal `ROLLED_BACK`, never `TARGET`, only when its linked terminal record has `terminal_status = ROLLED_BACK`, `closed_at <= expires_at`, matching pre/post-state fingerprints and positive restoration readback; a retry requires a new bounded receipt. A terminal `SUCCESS` is `TARGET` only when its readback deterministically matches the desired target and its complete GS-7 moving-base canary evidence is verified. A receipt closed after expiry is `DRIFT`, not a retroactively valid terminal state. Final V2 closeout rejects any remaining `PENDING` or `ROLLED_BACK` repository.
