...
For `CONTROL_PLANE_R2`, the owner authorization must be a new, unedited top-level comment on the canonical current PR/Issue by a human whose present repository-owner authority is verified through direct GitHub readback. The direct record must bind immutable comment ID, author login/type and independently read owner role, `created_at == updated_at`, repository, PR, exact material head, scope, and positive authorization, plus an independently read immutable creation event for that exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface). Bot/coordinator or ordinary-contributor text, a stale, edited, deleted/replaced, or audit-unanchored comment, or an actor whose owner role cannot be verified is `UNKNOWN`, never integration authority. This is an evidence constraint on the existing owner decision, not a second-human approval, a required status, or a new identity service.
