...
The V2 trusted boundary is an authenticated human repository owner and one serial settings writer. The owner decision must be evidenced by a new, unedited top-level comment on the canonical current PR/Issue from a human whose present repository-owner authority is directly verified in live GitHub state. The auditor/coordinator must verify the comment ID, author login and actor type, independently read owner role, `created_at == updated_at`, exact repository/PR/material head SHA/control-plane scope, and a positive integration authorization. Bot/coordinator or ordinary-contributor text, `author_association` by itself, a stale/head-mismatched, edited, missing, or unreadable comment, or an unreadable owner role is `UNKNOWN`, not owner authorization. Organization audit-log proof of exact comment creation/order is intentionally not required and must not be replaced with an attestation, audit, or proof subsystem. This protects the existing owner-decision boundary; it does not create a required status, second-human dependency, or new identity provider.
...
Transition evidence uses two distinct, unedited top-level records in the existing canonical rollout Issue/PR: a pre-transition record containing the fields above, created before the first mutation, and a terminal record containing `transition_id`, the GitHub `pre_transition_comment_id`, machine-readable `terminal_status` (`SUCCESS` or `ROLLED_BACK`), `post_state_fingerprint`, and `post_state_readback`. Direct GitHub readback must prove each record's unique comment ID, top-level placement, and `created_at == updated_at`. The one serial settings writer creates and reads back the pre-transition record before the first mutation; a terminal record can never validate a missing pre-transition record. The pre-transition and terminal GitHub creation timestamps, never values supplied in comment bodies, are the canonical `started_at` and `closed_at`. Missing, edited, duplicate, or mis-linked evidence is `DRIFT`; an unreadable required live field is `UNKNOWN` and blocks mutation. Terminal evidence is valid only when the derived `closed_at <= expires_at`; a late closure is `DRIFT`, not a retroactively valid terminal state. The auditor must recompute `post_state_fingerprint` from the readback. For `SUCCESS`, it must also prove the readback matches the repository's desired target and that the receipt's `success_condition` is satisfied by the complete GS-7 moving-base canary evidence, including unchanged candidate head, intervening `main` advance, exact merge-group SHA, successful aggregate-gate run, and resulting protected-main integration. A self-declared status, a free-form success note, or a passing normal PR check is insufficient. The auditor must use these fields and direct GitHub evidence, not expiry passage or mutable prose, to distinguish a closed receipt from an active deviation. Organization audit-log proof of comment creation/order is not a classification or integration predicate. These fields constrain the existing receipt and read-only auditor; they are not a new governance authority, required status, writer, database, or identity service.
...
A repository whose serial cutover has not begun is `PENDING`, not `TRANSITION` or `DRIFT`, only when the canonical rollout Issue/PR records a direct-readback pending baseline with `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback` in a distinct unedited top-level record. The read-only auditor must recompute the baseline fingerprint, direct-read its unique comment ID and `created_at == updated_at`, and compare current live state to it; a missing, edited, duplicate, malformed, or mismatching baseline is `DRIFT`. An unreadable required live field is `UNKNOWN`. The baseline is not a transition receipt and authorizes no V2 settings deviation. `PENDING` cannot satisfy target or terminal closeout and becomes `TRANSITION` only after a fresh matching readback and that repository's own bounded receipt exist immediately before its cutover.

A failed cutover is `ROLLED_BACK`, not `TRANSITION`, only when its linked unedited terminal record has `terminal_status = ROLLED_BACK`, timely `closed_at <= expires_at`, `post_state_fingerprint` matches `pre_state_fingerprint`, and positive `post_state_readback` proves restoration. `ROLLED_BACK` is terminal non-target evidence, authorizes no continued deviation or terminal closeout, and any retry requires a new bounded receipt; a missing or mismatched restoration proof is `DRIFT`.
...
For R2 control-plane changes covered by GS-5, owner authorization must be explicit and bound to the current material candidate. It is a new, unedited top-level comment on the canonical current PR/Issue by a human whose present repository-owner authority has been independently verified through direct GitHub readback. An executing bot/coordinator, ordinary contributor, task text, or `author_association` alone is not owner evidence.
...
repository
pull_request
current material head SHA
control-plane scope being changed
owner authorization to integrate this change
```

The direct readback must also record the comment ID, author login/type, independently verified owner role, `created_at == updated_at`, repository, PR, material head SHA, scope, and positive integration authorization; an edited or missing comment, unreadable or non-owner authority, stale scope, or head mismatch is invalid. A later material change invalidates that authorization. A pure target-base advance handled by Merge Queue does not require mutating the PR head solely to recreate authorization. If live owner-role or required comment verification is unavailable, classify this evidence `UNKNOWN` and stop integration.
