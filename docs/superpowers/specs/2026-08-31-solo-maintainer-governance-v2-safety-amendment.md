...
The owner decision must be evidenced by a new, unedited top-level comment on the canonical current PR/Issue from a human whose present repository-owner authority is directly verified in live GitHub state. The auditor/coordinator must verify immutable comment ID, author login and actor type, independently read owner role, `created_at == updated_at`, exact repository/PR/material head/control-plane scope, and a positive integration authorization. Bot/coordinator or ordinary-contributor text, `author_association` by itself, a stale/head-mismatched or edited comment, or an unreadable owner role is `UNKNOWN`, not owner authorization. This protects the existing owner-decision boundary; it does not create a required status, second-human dependency, or new identity provider.
...
A repository whose serial cutover has not begun is `PENDING`, not `TRANSITION` or `DRIFT`, only when the canonical rollout Issue/PR records a direct-readback pending baseline with `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback` in a distinct unedited top-level record. The read-only auditor must recompute the baseline fingerprint, direct-read its unique ID and `created_at == updated_at`, and compare current live state to it; a missing, edited, deleted, duplicate, malformed, or mismatching baseline is `DRIFT`. The baseline is not a transition receipt and authorizes no V2 settings deviation. `PENDING` cannot satisfy target or terminal closeout and becomes `TRANSITION` only after a fresh matching readback and that repository's own bounded receipt exist immediately before its cutover.
...
The direct readback must also record the immutable comment ID, author login/type, independently verified owner role, and `created_at == updated_at`; edited, deleted, unreadable, stale/head-mismatched, or non-owner artifacts are invalid. A later material change invalidates that authorization. A pure target-base advance handled by Merge Queue does not require mutating the PR head solely to recreate authorization. If live owner-role verification is unavailable, classify this evidence `UNKNOWN` and stop integration.
