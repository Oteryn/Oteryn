...
Record exact current `main` SHA, required contexts, strict-freshness setting, PR requirement, force-push/deletion state, approval/CODEOWNER requirement, bypass/admin enforcement, merge methods and Merge Queue state. Before any repository-content, workflow, branch, PR, settings, or break-glass mutation in a provider, record the current task's explicit authorization for that exact repository and permitted scope; absence or ambiguity keeps that repository read-only, and META authorization does not imply provider authorization. This scope record is not the `CONTROL_PLANE_R2` owner authorization required for a material integration.

Use one fresh, repository-scoped execution-routing packet per serial phase, never one organization-wide packet. Immediately before META implementation, and again immediately before each Game, Platform, and Atlas implementation/settings phase, refresh that repository's GitHub snapshot and validate its own packet with `python3 tools/governance/agent_execution_routing.py --policy ecosystem/agent-execution-routing-policy.json --packet <packet.json> --live-state <fresh-github-state.json>`. Execute the validator and policy only from an `Oteryn/Oteryn` checkout or trusted workflow verified at protected `main`; for Game, Platform, and Atlas, their own packet and fresh snapshot are inputs to META's validator, not files to copy into or execute from the provider. A provider or candidate-controlled copy must not be used as governance authority. In each packet, `execution_routing.github_preflight` contains only `verified_at`, `repository`, `default_branch_sha`, `governing_issue`, `pull_request`, and `task_head_sha`; execution target/runner, equivalent-CI availability, and host restrictions remain in `execution_routing`, while effort/decision basis, lane strategy, and the single settings-writer lane belong to `parallel_execution`. A passing packet for an earlier repository cannot cover a later one; resume or material head/state change requires a fresh validation for the affected repository. Any unreadable field remains `UNKNOWN` and blocks mutation of that field until readback is available.
...
Set the long-term `required_checks` arrays to exactly the four aggregate contexts above. Preserve main protection, squash-only, branch deletion-on-merge preference and security baseline. Extend the schema/validator only as much as necessary to represent `merge_queue`, transitional `strict_required_status_checks`, and explicit `PENDING` status for serially unstarted repositories without embedding transient PR/review data. Keep each `PENDING` baseline in the existing canonical rollout Issue/PR, not in desired state: it must be captured from direct live GitHub readback before classification and contain `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback`. The auditor must direct-read a unique, unedited top-level lifecycle record for the baseline (`created_at == updated_at`) and the immutable creation event for that exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface); missing, edited, deleted/replaced, duplicated, audit-unanchored, or unreadable evidence is `DRIFT`.
...
`PENDING` is not `TRANSITION`, requires no expiry receipt, and cannot permit a settings mutation or terminal V2 closeout. It is valid only when the canonical rollout Issue/PR has a direct-readback pending baseline containing `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback`, whose recomputed fingerprint and current live state match; missing, edited, deleted/replaced, duplicate, audit-unanchored, malformed, or mismatching evidence is `DRIFT`. It becomes `TRANSITION` only after a fresh matching readback, immutable audit-event anchor, and an unedited pre-transition top-level record are created immediately before its serial turn. Do not edit that record: terminal evidence is a separate, unedited top-level record linked by exact `transition_id` and GitHub `pre_transition_comment_id`; direct GitHub `created_at` values, never values supplied in comment bodies, are canonical `started_at`/`closed_at`, and `updated_at != created_at` is `DRIFT`. The pre-transition audit creation event must precede the first settings-mutation audit event; a terminal record cannot retroactively anchor the pre-transition record. `ROLLED_BACK` is terminal only when its linked terminal record has `terminal_status = ROLLED_BACK`, `closed_at <= expires_at`, `post_state_fingerprint` equals `pre_state_fingerprint`, and positive `post_state_readback` proves restoration; it cannot permit a settings mutation, retry, or terminal V2 closeout. The validator must never classify an explicitly authorized serial cutover as compliant before its repository canary succeeds. A timely `SUCCESS` is valid only when the auditor recomputes the post-state fingerprint, the readback matches the desired target, and the `success_condition` is evidenced by the complete GS-7 moving-base receipt (unchanged head, intervening main advance, exact merge-group candidate, successful aggregate gate, and protected-main integration). If the required existing audit surface cannot identify the exact comment/mutation events, classify `UNKNOWN` and do not mutate. The lifecycle metadata constraint reuses the canonical Issue/PR and read-only auditor; it creates no new database, status, writer, or merge authority.
