...
For automatic waiting/rotation mechanisms during `checkpoint_write`, require `snapshot["next_action"] == trusted_task.expected_next_action` and then `mechanism_verifier.is_live_and_bound(mechanism, locator, trusted_task, trusted_task.expected_next_action)`.
