...
- [ ] Add audit tests that a serially unstarted repository is explicit `PENDING`, not `DRIFT` or `TRANSITION`, only when a canonical-rollout pending baseline with `repository`, `captured_at`, `pre_state_fingerprint`, and `pre_state_readback` recomputes correctly and matches current live state; its unique top-level GitHub lifecycle record must direct-read `created_at == updated_at` and an immutable creation event for that exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface). Missing, edited, deleted/replaced, duplicate, audit-unanchored, malformed, or mismatching baseline is `DRIFT`. `PENDING` authorizes no settings deviation and blocks terminal V2 closeout.
...
- [ ] Add validation that an active transition is a unique, unedited top-level pre-transition GitHub lifecycle record with `transition_id`, `repository`, `issue_or_pr`, `expires_at`, `pre_state_fingerprint`, `allowed_deviations`, `success_condition`, and `rollback_condition`; direct readback must prove its ID and `created_at == updated_at`, derive canonical `started_at` solely from that server timestamp, and match an immutable creation event for the exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface). The first settings-mutation audit event must follow that pre-transition anchor; otherwise classify `DRIFT`/`UNKNOWN` before mutation.
- [ ] Add validation that terminal evidence is a separate unique, unedited top-level record linked by exact `transition_id` and `pre_transition_comment_id`, containing machine-readable `terminal_status`, `post_state_fingerprint`, and `post_state_readback`; direct readback must prove its ID and `created_at == updated_at`, derive canonical `closed_at <= expires_at` solely from that server timestamp, and match its immutable creation event. Recompute the post-state fingerprint from readback. A `SUCCESS` must additionally match the repository desired target and satisfy its `success_condition` through the complete GS-7 moving-base receipt, while `ROLLED_BACK` must match pre-state. Missing, edited, deleted/replaced, duplicate, mis-linked, audit-unanchored, or late evidence is `DRIFT`.
...
- [ ] Record owner authorization as a new, unedited top-level comment on the canonical current GitHub PR/Issue by a human whose present repository-owner authority is independently verified by direct live GitHub readback; before integration, direct-read the immutable creation event for that exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface). Bot/coordinator accounts, ordinary contributors, task text, and `author_association` alone are insufficient.
- [ ] Bind owner authorization to the current material head/equivalent immutable candidate coordinate.
- [ ] Record/validate authorization comment ID, author login/type, direct-read owner role, `created_at == updated_at`, repository, PR, material head, scope, positive integration authorization, and immutable audit-event ID/cursor, action, repository/PR, and server timestamp; edited, deleted/replaced, audit-unanchored, unreadable, stale, or head-mismatched evidence is `UNKNOWN` and blocks integration.
...
**Threat justification for the owner-authorship audit anchor:** the concrete threat is a coordinator, bot, or ordinary contributor posting, deleting, or replacing an authorization-shaped comment and treating its own text as the GS-5 owner decision. A current comment body, `author_association`, and candidate-controlled checks do not prove the actor's present owner authority or preserve the original authorization artifact. The minimal control is direct live verification of the human owner's role plus the immutable creation event for that exact existing PR/Issue comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface). Without it, a candidate-controlled mechanism can self-authorize integration. The operational cost is one owner-role and audit-event readback per `CONTROL_PLANE_R2` integration; its retirement condition is the durable integrated-PR evidence. It adds no required status, writer, database, merge authority, second-human dependency, or identity provider; if the existing audit surface cannot provide the evidence, integration stops instead of creating a new mechanism.
...
Before a live transition begins, create one durable, unedited top-level pre-transition record in the existing canonical rollout Issue/PR. Derive canonical `started_at` only from its GitHub server `created_at`; do not supply `started_at` in its body. Before the first settings mutation, direct-read and record the immutable creation event for that exact comment from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface), including event ID/cursor, repository/Issue/PR, action, and server timestamp. The body contains:
...
- [ ] Direct GitHub readback must prove both record IDs are present, unique, top-level, and unedited (`created_at == updated_at`); derive canonical `started_at` and `closed_at` solely from their respective GitHub creation timestamps, never from comment-body fields, and match each to its immutable existing GitHub audit-log creation event. The pre-transition anchor must precede the first settings-mutation audit event; terminal evidence cannot retroactively anchor it. Missing, edited, deleted/replaced, duplicate, mis-linked, audit-unanchored, or unreadable evidence is `DRIFT`; unavailable exact audit evidence is `UNKNOWN` and blocks mutation.
...
**Threat justification for the pending baseline and terminal evidence:** the concrete threats are false `DRIFT` after a completed serial cutover when `expires_at` later passes, expiry evasion through an edited, deleted/replaced deadline record or a late retroactive closure, false `PENDING` after an untouched provider's legacy settings drift, and false `SUCCESS` after a canary or target-state failure. Existing target state, active-receipt fields, mutable/free-form comments, and a terminal comment created after mutation cannot prove the original pre-mutation deadline, an unstarted repository's baseline, or that a closed success actually reached target through the moving-base canary. The minimal controls are direct GitHub readback of unique, unedited comment IDs/timestamps plus matching immutable creation and settings-mutation events from the existing GitHub audit log (or documented equivalent existing append-only GitHub audit surface) for the existing rollout Issue/PR baseline, pre-transition, and terminal records, plus the existing terminal readback/fingerprint and GS-7 receipt validation; without them an auditor can accept a replaced/late exception or misclassify drift. Their operational cost is one baseline record/audit readback per pending provider plus one pre-transition and one terminal record/audit readback per cutover; their retirement condition is terminal closeout, with records retained only as evidence. They add no new status, writer, database, merge authority, or identity provider. If the existing audit surface cannot provide the exact evidence, mutation is blocked rather than adding a new governance mechanism.
