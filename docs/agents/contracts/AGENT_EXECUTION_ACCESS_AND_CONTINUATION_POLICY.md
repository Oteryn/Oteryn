...
`single_agent` is a normal first-class strategy and requires exactly one lane; it does not require a serial-exception reason. Use `parallel_when_beneficial` only when at least two materially independent workstreams can make concurrent progress and the expected benefit exceeds coordination and integration cost. When parallelism is beneficial, use the smallest useful number of lanes. Each lane declares an ID, owned repository-relative paths, dependencies, separate `branch` and `worktree` identities, and shared leases. Routing schema 3 requires `branch` as the full `refs/heads/...` ref and `worktree` as a canonical, execution-host-qualified workspace identity. Resolve filesystem aliases, symlinks and path casing on the execution host before recording that identity. The allocator must use the same identity for the same physical worktree. The offline validator rejects either repeated identity independently and rejects the retired ambiguous `branch_and_worktree` field; it does not replace actual checkout/ownership verification or implement a runtime lock.
