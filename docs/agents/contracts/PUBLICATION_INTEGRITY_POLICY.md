...
The guarded push uses `--no-verify` so repository-local `pre-push` hooks or a configured hooks path cannot perform additional, unreviewed publication side effects. Hooks may remain useful for ordinary developer workflows, but they are not part of the authorized exact-candidate transport boundary.
