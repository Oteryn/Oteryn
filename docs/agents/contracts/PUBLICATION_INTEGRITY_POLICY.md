...
- repository plus exactly one configured, credential-free approved push target that uses a fixed native push-capable URL scheme (`ssh`, `git`, `http`, `https`, or `file`) or a native local/scp-like form, and does not use explicit Git remote-helper `<transport>::<address>` syntax;
