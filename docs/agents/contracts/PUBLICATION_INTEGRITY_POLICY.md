...
The selected remote must resolve to exactly one configured push URL and that URL must equal the approved publication target. Effective Git URL rewrite rules are rejected rather than reimplemented or partially interpreted. An endpoint string that is itself another configured remote name is rejected rather than left to Git's remote-name/URL ambiguity. Repository/worktree credential helpers are rejected before remote readback and rechecked before mutation, including helper values loaded through local includes, so repository-controlled credential code cannot execute before recovery preservation or during the guarded push. Authorized system/global credential helpers or environment-backed authentication remain compatible because they are outside repository/worktree control. Preflight readback, mutation and post-mutation readback all use the same resolved push endpoint; a distinct fetch URL is not publication evidence, multiple push URLs fail closed before mutation, and no `insteadOf`/`pushInsteadOf` chain may redirect the endpoint.
