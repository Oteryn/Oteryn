...
The worker-release authority function accepts fresh typed
`AcquiredCapabilityEvidence` assembled from current-session tool/action discovery
and protected-executor readback. Hosts that can install `TrustedCapabilityObserver`
may continue to use that compatibility adapter, but the wrapper is not itself a
required authority boundary. A `Mapping`, JSON document, caller string/boolean,
comment, serialized fixture, or previous-session assertion is not capability
evidence. Malformed typed evidence fails closed.
...
`main` SHA from the validated evidence, and canonical control-request construction
...
Current-session tool discovery obtains the actual human actor identity used by its
control-comment operation, and protected-executor readback obtains the human
principal identity of the fine-grained mutation credential. Delegated capability
requires both identities to be present, well formed and equal. A caller-provided
actor assertion without the corresponding live readback is not sufficient. This
check is independent of the direct-native route and cannot block an otherwise
verified direct capability.
...
closed because no current-session typed capability evidence is supplied there.
...
`DIRECT_CAPABLE` means current-session discovery observed the execution surface
exposing the governed native operation selected by META:
