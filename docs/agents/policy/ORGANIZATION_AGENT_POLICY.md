...
Protected GitHub enforcement and Merge Queue, where configured, remain integration authority. A submission capability is acceptable only when its mutation is queue-specific and server-side fenced to the exact frozen candidate. Prefer a native `enqueuePullRequest` operation with `expectedHeadOid` equal to the exact qualified PR head. A protected-default-branch executor is an equivalent repository-native route only when trusted default-branch code re-reads the same-repository PR, requires base `main`, verifies the exact current head and the required exact-head aggregate gate, and then performs only GraphQL `enqueuePullRequest` with `expectedHeadOid` set to that same frozen SHA. It must require a returned Merge Queue entry, fail closed on any mismatch or GitHub/API error, and must not execute candidate-controlled code or gain direct-merge, bypass, approval, protection-changing or unrelated repository-write authority. Use the least-privilege short-lived credential capable of the required queue mutation.

`enablePullRequestAutoMerge` is not an enqueue capability. Even when a Merge Queue currently exists, generic auto-merge cannot safely substitute for queue-specific enqueue because branch/queue state can change before the mutation and a connector surface may not expose an atomic exact-head fence. No bypass, direct merge or generic auto-merge substitutes for an unavailable enqueue tool. If neither an authorized exact-head-fenced native enqueue action nor an authorized protected-default-branch executor is callable in the current execution surface, integration is blocked at enqueue; continue other safe path-disjoint work rather than weakening Merge Queue authority. After enqueue, require the repository's normal Merge Queue/merge-group gate and protected-main readback before declaring integration complete. Historical `added_to_merge_queue` timeline events alone are not terminal integration proof. ADR 0005's retired custom review fingerprints, envelopes, attestations and `ai-review-gate` must not be recreated as merge authority.
