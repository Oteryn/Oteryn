...
Protected GitHub enforcement and Merge Queue, where configured, remain integration authority. Before selecting an autonomous integration route, require fresh target-bound authorization/eligibility for the exact repository, PR number, base branch and qualified/frozen head. The queue mutation itself must provide an **atomic head-and-base/queue fence**: the same server-side operation that admits the PR must reject both a moved head and a retargeted base/queue. A preceding base read, a later queue readback, or cleanup after a mismatched mutation does not satisfy this requirement. Post-mutation readback cannot repair an already-applied wrong-base enqueue safely under concurrency.

Current documented GitHub primitives do not provide that complete fence. REST `merge-async` can bind the head with `sha` and explicitly select `merge_action="merge_queue"`, but it does not expose an expected base/queue precondition. GraphQL `enqueuePullRequest` can bind `expectedHeadOid`, but `EnqueuePullRequestInput` likewise does not expose an expected base/queue precondition. GraphQL `dequeuePullRequest` targets the pull request ID rather than a specific observed queue-entry ID, so a read-then-dequeue repair has its own replacement race. These surfaces are useful evidence and may become sufficient if GitHub adds the missing atomic coordinate, but under the current contract they are not governed autonomous submission capabilities for an exact-`main` candidate.

A future connector, GitHub API revision, or protected-default-branch executor may be used only when its underlying mutation atomically enforces both the exact qualified head and expected `main` base/queue. Trusted execution location, short-lived credentials, exact queue receipts, or `clientMutationId` correlation do not substitute for a missing mutation-time base fence. Do not create or operate a privileged enqueue broker whose safety depends on post-hoc dequeue of an ambiguously attributable/current queue entry. **Do not model an unknown future capability as a caller-asserted boolean or self-asserted receipt.** A new positive route requires a reviewed repository-source change that identifies the concrete GitHub primitive and its mutation-time fence.

`enablePullRequestAutoMerge` is not a governed agent enqueue capability. It may remain a GitHub UI/CLI convenience where GitHub itself applies repository rules, but a generic connector wrapper without explicit queue mode and exact candidate fencing must not substitute for the governed route. `direct_merge`, immediate merge APIs, bypass, force push, no-op/retrigger commits and protection changes are also forbidden substitutes. When no callable route supplies the required atomic head-and-base/queue fence, integration is `BLOCKED_CAPABILITY_UNAVAILABLE`; continue other safe path-disjoint work rather than weakening Merge Queue authority. A submission/queue receipt is never terminal integration proof by itself: normal `merge_group` aggregate-gate success and protected-main readback remain required. Historical `added_to_merge_queue` events alone are not terminal proof. ADR 0005's retired custom review fingerprints, envelopes, attestations and `ai-review-gate` must not be recreated as merge authority.
