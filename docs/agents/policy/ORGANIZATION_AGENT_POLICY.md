...
Mitigate the missing mutation-time base fence with mandatory fresh target validation immediately before and immediately after accepted submission. The preflight must bind repository, PR number, `base=main`, exact head, authorization and eligibility. After HTTP `202`, immediately refresh the live PR and require the same repository, PR number, `base=main` and exact head before any autonomous lifecycle closeout continues. Post-mutation readback is detection rather than retroactive atomic prevention; if it observes a changed base/head or stale state, stop with a target-mismatch blocker and require human reconciliation. Do not issue ambiguous automated `dequeuePullRequest` cleanup.
