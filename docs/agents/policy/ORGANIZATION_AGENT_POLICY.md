...
Before releasing substantial mutating work that is expected to require autonomous protected integration, the active scheduling/control-plane agent must classify the current integration execution path through `docs/agents/contracts/INTEGRATION_CAPABILITY_ROUTING_POLICY.md` and `tools/governance/integration_capability_routing.py`. The worker-release entry point consumes fresh typed `AcquiredCapabilityEvidence` assembled from current-session tool/action discovery and protected-executor readback; a dedicated `TrustedCapabilityObserver`, sealed observation wrapper, caller-created mapping, JSON snapshot, operation string, timestamp, label or boolean is not required or accepted as a substitute. `DIRECT_CAPABLE` uses the native exact-head `merge-async` operation discovered on the current session; `DELEGATED_CAPABLE` may use the protected META executor only after fresh evidence proves that executor operational and preserves its exact canary-qualified protected-META `main` binding. Capability never grants or transfers merge authority. Missing, malformed, stale or incomplete evidence records `BLOCKED_CAPABILITY_UNAVAILABLE` **before worker release** rather than discovering the limitation only after implementation is complete.
