...
Protected GitHub enforcement and Merge Queue, where configured, remain integration authority. Prefer an explicit native enqueue action when one is exposed and authorized. When the live target branch requires GitHub Merge Queue and an explicit enqueue action is unavailable, GitHub-native `enablePullRequestAutoMerge` is an allowed queue-submission route for an otherwise authorized and eligible PR only when direct GitHub post-action evidence proves queue admission, such as `added_to_merge_queue`, a queue entry or a `merge_group` candidate. That route is an integration convenience under ADR 0005, not a direct-merge fallback or protection bypass; if Merge Queue is not required or queue admission cannot be proven, do not claim queue submission and fail closed on the unresolved integration state. No bypass or direct merge substitutes for an unavailable enqueue tool. ADR 0005's retired custom review fingerprints, envelopes, attestations and `ai-review-gate` must not be recreated as merge authority.
