...
If the execution surface does not expose safe `merge-async`, fall back only to another queue-specific exact-head-fenced operation: native GraphQL `enqueuePullRequest` with `expectedHeadOid` equal to the exact qualified PR head, or the protected-default-branch META submission executor at Issue #190. The permanent agent-callable command is `/oteryn-mq-submit <attempt_id> <owner/repo> <pr_number> <exact_head_sha>`. The protected workflow must authenticate the current control-endpoint actor, allow only the four permanent Oteryn repositories, re-read the same-repository open non-draft PR with base `main`, exact current head and required exact-head aggregate gate, and mint a short-lived GitHub App installation token scoped to that single target repository only after authorization. Its only integration mutation is GraphQL `enqueuePullRequest(expectedHeadOid=<exact head>)`; it requires the exact returned Merge Queue entry and queue identity, is idempotent for an already-enqueued exact head, executes no candidate-controlled code, and gains no direct-merge, bypass, approval, protection-changing or unrelated repository-write authority. Success receipts prove submission only; normal `merge_group` validation and protected-main readback remain terminal authority.
