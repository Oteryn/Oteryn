...
For publication of an already-prepared material local candidate, `docs/agents/contracts/PUBLICATION_INTEGRITY_POLICY.md` and `docs/agents/operations/RESTRICTED_PUBLISHING.md` own exact-candidate preservation, recovery and credential compatibility. Bind the candidate and expected remote predecessor, preserve a verified recoverable Git object artifact before disposable-workspace release, use the normal non-force Git publication path, and require live remote-head readback. If that route is unavailable, do not reconstruct the candidate through ad-hoc Git Data object/ref writes or per-file API substitutes. A failed or timed-out push must be classified by remote readback before retry; a third remote SHA fails closed as ownership/state drift. `tools/governance/publication_integrity.py` is the deterministic local helper when a Git workspace is available; it is not a second merge authority.
