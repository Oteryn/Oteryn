...
Protected GitHub enforcement and Merge Queue, where configured, remain integration authority. Prefer an explicit native enqueue action when one is exposed and authorized. When explicit enqueue is unavailable, GitHub-native `enablePullRequestAutoMerge` may be used as a Merge Queue submission convenience only as one bounded submission attempt. After exact-head qualification and freeze, post exactly `/oteryn-mq-preflight <attempt_id> <owner/repo> <pr_number> <expected_head_sha>` to the permanent META control Issue `Oteryn/Oteryn#189`. The protected default-branch META preflight must reject the command unless the comment actor currently has `write`, `maintain` or `admin` permission on the META control repository, then re-read the exact target PR/head and exact target Merge Queue. Consume only the resulting fresh `OTERYN_MQ_PREFLIGHT_V1` GitHub-authored proof comment after independently verifying that the proof comment is authored by `github-actions[bot]`, its proof/comment/run identifiers match, and the referenced protected preflight `issue_comment` workflow run on META `main` completed successfully at the exact workflow SHA carried by the proof. The proof must bind control repository/Issue, target repository, PR number, base branch, current head SHA and exact Merge Queue identity, including queue id, resource path and URL, and it expires after the machine-defined short freshness window. An earlier PR/merge-group CI run, cached protection read, task note, user-authored lookalike proof comment or historical timeline event is not submission authorization. While that exact proof is still fresh, perform a fresh connector live-head read bound to the same target repository/PR/base and frozen SHA; only if that read is within the machine-defined live-head freshness window and does not move may the connector invoke the target repository's native auto-merge action. Do not substitute a direct merge call. Record the connector-return time. Queue admission is terminally proven only by a fresh current queue-entry readback or a merge-group observation collected strictly after the connector mutation returns and bound to the same attempt and exact target PR/head. Historical `added_to_merge_queue` timeline events alone are not terminal admission proof. If actor permission cannot be verified, the META preflight cannot prove required Merge Queue, proof provenance/run success cannot be authenticated, the proof expires, the fresh connector live-head read differs or becomes stale, the connector mutation is unavailable/fails, or post-mutation admission cannot be proved, fail closed and do not reinterpret ordinary auto-merge as queue admission. This route is an integration convenience under ADR 0005, not a direct-merge fallback or protection bypass. No bypass or direct merge substitutes for an unavailable enqueue tool. ADR 0005's retired custom review fingerprints, envelopes, attestations and `ai-review-gate` must not be recreated as merge authority.
