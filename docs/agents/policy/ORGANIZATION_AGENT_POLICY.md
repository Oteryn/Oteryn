...
Before releasing substantial mutating work that is expected to require autonomous protected integration, the active scheduling/control-plane agent must classify the current integration execution path through `docs/agents/contracts/INTEGRATION_CAPABILITY_ROUTING_POLICY.md` and `tools/governance/integration_capability_routing.py`. The worker-release entry point must acquire evidence through an installed trusted observer; caller-created mappings, JSON snapshots, operation strings, timestamps, labels, or booleans are never authority. `DIRECT_CAPABLE` uses the native exact-head `merge-async` operation discovered on the current session; `DELEGATED_CAPABLE` may use the protected META executor only after trusted live readback proves that executor operational. Capability never grants or transfers merge authority. If no trusted observer is installed or neither route is proven, record `BLOCKED_CAPABILITY_UNAVAILABLE` **before worker release** rather than discovering the limitation only after implementation is complete.
