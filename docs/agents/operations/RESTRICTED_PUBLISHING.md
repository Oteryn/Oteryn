...
Use `tools/governance/publication_integrity.py` when an isolated Git workspace is available. It requires the selected remote to resolve to exactly one credential-free configured push URL matching the approved target; rejects an endpoint that is itself another configured remote name; rejects effective `url.*.insteadOf` / `url.*.pushInsteadOf` rules; rejects effective executable `filter.*.clean` / `filter.*.process` configuration; and rejects replacement refs, graft metadata and replacement-ref namespace overrides before ancestry is trusted. It rejects tracked `skip-worktree` / `assume-unchanged` flags that can hide local bytes, verifies the expected remote-head fence, candidate ancestry and clean exact-candidate worktree; runs the clean-worktree probe with optional index writes disabled, `core.fsmonitor=false`, and `core.hooksPath` bound to a fresh trusted empty directory so repository-controlled filters, fsmonitor or index hooks cannot side-publish; creates and verifies a recoverable Git bundle before mutation; performs one exact-candidate update guarded by an expected-old-value lease; bypasses `pre-push` hooks for that controlled mutation; and reads the branch back through the same push endpoint before reporting success.

A separate fetch URL is not publication evidence, multiple push URLs fail closed before mutation, Git URL rewrites are publication blockers until removed from the effective configuration, executable clean/process filters are publication blockers until removed from the exact-candidate workspace configuration, replacement/graft history cannot authorize ancestry, hidden index flags cannot suppress local recovery state, and repository filters/fsmonitor/index/pre-push hooks are not allowed to add side-effect pushes to the guarded publication operation.

The helper's explicit `--force-with-lease=<ref>:<expected_sha>` is used only as compare-and-swap protection for the expected predecessor. Candidate ancestry is checked independently first with replacement objects disabled, so this does not authorize a non-fast-forward update or history rewrite. An agent must not use the option without those guard conditions or treat it as a general force-push exception.
