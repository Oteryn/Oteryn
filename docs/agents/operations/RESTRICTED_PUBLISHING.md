...
For an authorized write to that branch/PR, when `GH_TOKEN` and `GITHUB_TOKEN` are unset but agent-visible `GH` is present, it may be passed transiently as `GH_TOKEN="$GH"` to the exact authorized `gh` command. Environment mapping alone does not authenticate `git push`: verify that the existing system/global/environment credential path can consume the authorized identity without exposing or persisting it. Repository/worktree `credential.helper` configuration is intentionally not accepted by the exact-candidate publisher because it is repository-controlled executable configuration. If the normal Git publication path is still unavailable, preserve the candidate/recovery artifact and report the exact limitation rather than inventing a lower-level publication route.

Never embed the token in a remote URL or persist a new repository/worktree credential helper to bypass that boundary. Credential presence expands no repository/path/task/merge/production permission. Do not perform a non-fast-forward update; verify the remote exact head after publishing.
