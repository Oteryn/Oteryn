...
The authorization is valid only when it is a new, unedited top-level comment on the canonical current PR/Issue by a human whose present repository-owner authority is verified by direct GitHub readback. Record and verify the immutable comment ID, author login and actor type, the independently read owner role, `created_at == updated_at`, repository, PR, exact material head, control-plane scope, and positive integration authorization. Before integration, direct-read an immutable creation event for that exact comment ID from the existing GitHub audit log (or a documented equivalent existing append-only GitHub audit surface), and record its event ID/cursor, repository/PR, action, and server timestamp. A bot, coordinator, ordinary contributor, stale/head-mismatched, edited, deleted/replaced, or unverifiable actor/artifact is `UNKNOWN`, not owner authorization; a comment's text or `author_association` alone is insufficient.
...
Use the canonical rollout Issue/PR as the durable transition receipt, using distinct, unedited top-level comments rather than editing a record in place. The auditor derives canonical `started_at` solely from the pre-transition comment's GitHub server `created_at`; never put `started_at` in the comment body. Before the first mutation, direct-read and record an immutable creation event for that exact pre-transition comment ID from the existing GitHub audit log (or a documented equivalent existing append-only GitHub audit surface), including event ID/cursor, repository/Issue/PR, action, and server timestamp. The pre-transition comment, created before the first mutation, must record at least:
...
The read-only auditor must directly read both GitHub comments and accept them only when their IDs are present and unique, they are top-level records, `created_at == updated_at`, and the pre-transition `created_at` and terminal `created_at` are the canonical `started_at` and `closed_at` respectively. The comments must not supply either timestamp field. Each record must also match its independently read immutable creation event, and the first settings-mutation audit event must be later than the pre-transition creation event; the terminal record is not an anchor for the pre-transition record. A missing, deleted, edited, duplicated, replaced, mis-linked, audit-unanchored, or unreadable record is `DRIFT`; the same unedited-ID/timestamp/audit-anchor proof is required for a `PENDING` baseline. If the required existing audit surface cannot be read or does not identify the exact comment and mutation events, classify `UNKNOWN` and do not mutate. This constrains the existing lifecycle record and read-only auditor; it is not a new status, writer, database, or merge authority.
