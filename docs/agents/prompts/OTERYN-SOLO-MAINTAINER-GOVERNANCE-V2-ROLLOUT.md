...
5. Before canary admission, prove `provenance-gate` is produced on the exact merge-group candidate. If it is not, only after the replacement path is proven and explicit `CONTROL_PLANE_R2` owner authorization is recorded for the current material head SHA/live PR, use one bounded transition to remove that legacy context, record the allowed deviation and readback, and restore it immediately if the canary fails.
