...
3. **explicit owner authorization bound to the current material head SHA / live PR state**;
...
The authorization is valid only when it is a new, unedited top-level comment on the canonical current PR/Issue by a human whose present repository-owner authority is verified by direct GitHub readback. Record and verify the comment ID, author login and actor type, the independently read owner role, `created_at == updated_at`, repository, PR, exact material head SHA, control-plane scope, and positive integration authorization. A bot, coordinator, ordinary contributor, stale/head-mismatched, edited, missing, or unverifiable actor/artifact is `UNKNOWN`, not owner authorization; a comment's text or `author_association` alone is insufficient. The V2 trusted boundary is the authenticated human owner and one serial settings writer: organization audit-log proof of exact comment creation/order is intentionally not required, and no replacement attestation, audit, or proof subsystem may be created.
...
5. Before canary admission, inspect the trusted protected-`main` workflow trigger/configuration and current required-context readback to determine whether legacy `provenance-gate` can run on `merge_group`. Do not require an exact synthetic candidate as the precondition for that decision when the missing legacy context can itself block admission. If protected-source/live evidence shows that `provenance-gate` lacks `merge_group` support, or cannot establish that support without such a deadlock, only after the replacement path is proven and explicit `CONTROL_PLANE_R2` owner authorization is recorded for the current material head SHA/live PR, use one bounded transition to remove that legacy context, record the allowed deviation and readback, and restore it immediately if the canary fails. The admitted canary then supplies the required exact synthetic-candidate runtime evidence for `atlas-gate` and its internal provenance validation.
...
Use the canonical rollout Issue/PR as the durable transition receipt, using distinct, unedited top-level comments rather than editing a record in place. The auditor derives canonical `started_at` solely from the pre-transition comment's GitHub server `created_at`; never put `started_at` in the comment body. The one serial settings writer must create and directly read back the pre-transition comment before the first mutation. The pre-transition comment must record at least:
...
The read-only auditor must directly read both GitHub comments and accept them only when their IDs are present and unique, they are top-level records, `created_at == updated_at`, and the pre-transition `created_at` and terminal `created_at` are the canonical `started_at` and `closed_at` respectively. The comments must not supply either timestamp field. The one serial settings writer creates and reads back the pre-transition comment before mutation; the terminal record cannot retroactively validate a missing pre-transition comment. A missing, edited, duplicated, or mis-linked record is `DRIFT`; an unreadable required live field is `UNKNOWN` and blocks mutation. The same unedited-ID/timestamp direct-readback rule is required for a `PENDING` baseline. This trusted-boundary model does not require independent audit-log proof of comment creation or mutation ordering. This constrains the existing lifecycle record and read-only auditor; it is not a new status, writer, database, or merge authority.
