...
When `activation.allow_clean_trusted_base_merge_reuse` is enabled, a final sequence of clean two-parent integration merges may reuse the review only when the final merge's second parent is the exact trusted PR base, each earlier integration merge's second parent is a distinct ancestor of that trusted base, every first parent descends from the reviewed head, Git can reproduce every merge tree conflict-free from its two parents, and all first-parent commits after review are otherwise review-neutral. This preserves valid review evidence across repeated unrelated `main` advances without allowing a duplicate merge-up against the same base. An exact-head review of any resulting integration merge is valid without ancestry traversal.
...
`ACCEPTED_WITH_FOLLOW_UP` is the only non-clean outcome that may satisfy the gate. It is not a clean result: P0/P1, escalated or unclassified findings always block. Every current-generation trusted P2 root finding must be unedited, be in a resolved GraphQL review thread, and have exactly one unedited reply from an `OWNER`, `MEMBER` or `COLLABORATOR` whose entire body is `Tracked in #<issue>.`. The referenced object must be an open Issue—not a pull request—in the same repository. Missing, duplicate, edited, stale, cross-repository, closed or paginated evidence fails closed. The verifier emits the explicit outcome together with exact finding, thread and Issue identifiers for downstream attestation.

After the `Tracked in #<issue>.` reply, review-thread resolution, and open follow-up Issue are all complete, the closeout coordinator MUST publish `<!-- OTERYN_AI_REVIEW_EVIDENCE_WAKEUP_V1 -->` as a top-level pull-request conversation comment. This comment is a wake-up producer only: it causes the same-head recheck to re-read the complete live evidence envelope, and it grants no review authority by itself. The marker is never a substitute for the trusted P2 finding, resolved thread, maintainer disposition, open Issue, immutable request anchor, exact tier/fingerprint, or any other verifier predicate.
...
META additionally owns `.github/workflows/governance-ai-review.yml`, triggered by `pull_request_target`, and `.github/workflows/governance-ai-review-request.yml`, triggered by trusted-association PR Conversation comments. The gate workflow checks out only the exact protected base SHA with credentials disabled. Before it reads any candidate object, trusted-base code re-fetches and binds the repository ID and exact same-repository PR/base/head from GitHub. It then fetches the exact base/head only into a bare Git object store for classifier and ancestry reads; it never creates a candidate worktree or executes candidate scripts. The in-job `github.sha`, `github.workflow_sha` and `github.workflow_ref` must all bind trusted base `T`; REST workflow-run/job/check-suite facts intentionally bind candidate head `H` and one exact same-repository PR association, never workflow provenance. Before issuance and again after attestation verification, trusted-base code recomputes policy ID/raw digest, classifier revision, tier/fingerprint/reviewer class/evidence status and normalized evidence source from trusted `T` plus inert `T/H` objects. The request-registry workflow executes only default-branch inline code, records immutable submitted-review anchors, and never checks out the candidate. Cross-repository PRs fail closed. This trusted-base check is the post-bootstrap merge authority for AI review evidence.

The fake-Git regression suite establishes command and secret-boundary behavior only; it is not a substitute for private GitHub-hosted authorization evidence. Before any future private-repository adoption or cutover, a same-repository private canary PR must retain its exact run URL, show the `ai-review-gate` bare fetch and attestation verification completed under the job-scoped token, and be read back for the expected run/job/check chain. The canary must be treated as failed if output, artifacts or Git configuration expose credential material; a simulated local success cannot satisfy this evidence requirement.

For every trusted gate evaluation, the gate emits canonical JSON bytes under predicate type `https://oteryn.dev/bounded-execution-evidence/v1`. The envelope binds repository ID/name, PR number and base/head, policy ID/content SHA-256, classifier revision SHA-256, tier/fingerprint, immutable typed review-source coordinates, issuer workflow ref/SHAs, run/attempt/job/check-suite/check IDs and `issued_at`. Its `sha256` is the binding/envelope identifier. The workflow uses pinned `actions/attest@1e69f48acb82d1966a394da916b4c1698aa569d6` to attest those exact bytes both as subject and custom predicate, with `actions: read`, `checks: read`, `contents: read`, `issues: read`, `pull-requests: read`, `id-token: write`, `attestations: write` and `artifact-metadata: write` only. It immediately verifies the issued record with `gh attestation verify` constrained to repository, predicate type, signer workflow/digest `T`, source ref/digest `T` and GitHub-hosted runners. Trusted-base code then requires the returned Fulcio certificate's signer/config/source digests and workflow/source refs to be exactly `T`, plus the exact run invocation URI, and re-reads the live PR/run/job/check/source coordinates. Any API, pagination, identity or attestation ambiguity fails closed.

- `R0`: pass after deterministic checks; the envelope carries `evidence_status: not_required` and no external AI is requested.
- `R1`/`R2` while Draft: report tier/fingerprint and pass so deterministic CI can stabilize without spending AI quota; the envelope carries `evidence_status: deferred` and no review source.
- `R1`/`R2` when Ready: fail closed until trusted authenticated external `PASS` or fully validated `ACCEPTED_WITH_FOLLOW_UP` evidence matches the current fingerprint. A deep reviewer may satisfy a fast-review requirement; a fast reviewer never satisfies `R2`.
